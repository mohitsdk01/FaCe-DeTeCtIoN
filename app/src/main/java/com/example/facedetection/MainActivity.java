...
public class MainActivity extends AppCompatActivity implements FrameProcessor {
        ...
        faceDetector.detectInImage(firebaseVisionImage)
                .addOnSuccessListener(new OnSuccessListener<List<FirebaseVisionFace>>() {
                    ...
                    public void onSuccess(List<FirebaseVisionFace> firebaseVisionFaces) {
                        Bitmap mutableImage = bitmap.copy(Bitmap.Config.ALPHA_8, true);
                ...
                .addOnFailureListener(new OnFailureListener() {
                    ...
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(MainActivity.this, "There war an error", Toast.LENGTH_SHORT).show();
                        hideProgress();
    ...
    private void detectFaces(List<FirebaseVisionFace> firebaseVisionFaces, Bitmap bitmap) {
        ...
        for (int i = 0; i < firebaseVisionFaces.size(); i++) {
            ...
            canvas.drawText("Face " + i,
                    (firebaseVisionFaces.get(i).getBoundingBox().centerX() - (firebaseVisionFaces.get(i).getBoundingBox().width() >> 1) + 8f),
                    (firebaseVisionFaces.get(i).getBoundingBox().centerY() + (firebaseVisionFaces.get(i).getBoundingBox().height() >> 1)) - 8f, facePaint);
            ...
            if (face.getLandmark(FirebaseVisionFaceLandmark.LEFT_EAR) != null) {
                ...
                assert leftEar != null;
                canvas.drawCircle(Objects.requireNonNull(leftEar).getPosition().getX(), leftEar.getPosition().getY(), 8f, landmarkPaint);
            }
    ...
    public void process(@NonNull Frame frame) {
        ...
        FirebaseVisionImageMetadata metadata = new FirebaseVisionImageMetadata.Builder().setWidth(width).setHeight(height).setFormat(FirebaseVisionImageMetadata.IMAGE_FORMAT_NV21)
                .setRotation(
